...
from models.blocks import CA

class SS2D(nn.Module):
    r"""2D Selective Scan Module"""
    def __init__(
            self,
            d_model,
            d_state=16,
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        # self.show_feature_map(H, v.view(B, -1, H, W), 'Attention/v')
        # self.show_feature_map(H, h.view(B, -1, H, W), 'Attention/h')

        y = self.fusion_proj(torch.cat((v, h), 1).permute(0, 2, 3, 1).contiguous())
        # self.show_feature_map(H, y.permute(0, 3, 1, 2), 'Attention/vh')
    ...
    # def show_feature_map(self, size, feature_map,
    #                      path):  # feature_map=torch.Size([1, 64, 55, 55]),feature_map[0].shape=torch.Size([64, 55, 55])
    #     # feature_map[2].shape     out of bounds
    #     # ph_num = photo_num
    #     feature_map = feature_map.squeeze(0)  # torch.Size([64, 55, 55])
    ...
    #     # plt.figure()
    #     for index in range(1, feature_map_num + 1):  # 64 tensor
...
class ASSBlock(nn.Module):
    ...
    def __init__(
        ...
        self.sa = SS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
        self.drop = DropPath(drop_path)

        self.p = nn.Parameter(torch.ones(hidden_dim))

    def forward(self, input):
        ca = input+self.ca(self.norm1(input))
        sa = ca+self.drop(self.sa(self.norm2(ca)))
        out = input*self.p + sa
        return out

class ASSModule(nn.Module):
    """ An Adaptive State-Space Module of Mamba."""
    def __init__(
            self,
            dim,
            depth,
